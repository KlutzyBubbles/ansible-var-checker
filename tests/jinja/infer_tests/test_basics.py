...
def test_basics_5():
    ...
    struct = infer(template)
    expected_struct = Dictionary({
        'items': List(List(Dictionary({
            'x': Scalar(label='x', linenos=[4])
        }, label='column', linenos=[4]), label='row', linenos=[2, 3]), label='items', linenos=[2]),
    })
    assert struct == expected_struct
...
def test_assignment():
    ...
    struct = infer(template)
    expected_struct = Dictionary({
        'args': List(Scalar(linenos=[2, 3, 4]), label='args', linenos=[2, 3, 4, 5]),
